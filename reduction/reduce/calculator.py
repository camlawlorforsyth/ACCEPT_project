...
Original Javascript version © 1999-2016 Edward L. Wright. For further information, please see http://adsabs.harvard.edu/abs/2006PASP..118.1711W
'''
...
ouput values = age at z, comoving radial distance in Mpc, scale of ___ kpc/arcsec, apparent to abs mag conversion
...
# if Univ is General, use Ho, Wm and given Wv
    ...
    WM = float(sys.argv[3+verbose]) # Omega(matter)
    WV = float(sys.argv[4+verbose]) # Omega(vacuum) or lambda
    R_out = float(sys.argv[5+verbose])
...
# comoving volume computation
  ...
  if verbose == 1:
    ...
    print 'The luminosity distance D_L is ' + '%1.3f' % DL_Mpc + ' Mpc or ' + '%1.1f' % DL_Gyr + ' Gly.'
    print 'The distance modulus, m-M, is '+'%1.2f' % (5*log10(DL_Mpc*1e6)-5)
  else:
    print 'z: %1.4f' % z
    if R_out != 0:
        print 'R_out in Mpc: %1.3f' % R_out
    print 'D_A in Mpc: %1.3f' % DA_Mpc
    print 'Minimum number of points/counts: %1.3f' % (20000 * ((1.0+z)**4))
    if R_out != 0:
      rad = ((float(R_out)/float(DA_Mpc)) * (180.0/math.pi))
      print 'Radius: %1.8f' % rad
