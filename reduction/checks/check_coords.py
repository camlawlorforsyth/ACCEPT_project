...
python checks/check_coords.py 1E_0657-56 complete
...
if quality == "complete" :
    ...
    if (not os.path.isfile("ds9_fk.reg")) :
        file.write(cluster + " is labelled correctly," +
                   " but 'ds9_fk.reg' is missing.\n")
    else :
        newfile = open("ds9_fk.reg", "r") # open for reading
        newfile.readline() # read the first line of ds9_fk.reg
        f6 = newfile.read(6) # read the first 6 characters
        if (f6 != "global") :
            file.write(cluster + "'s ds9_fk.reg file is saved in the wrong " +
                       "coordinates.\n")
        newfile.close()
    ...
    if (not os.path.isfile("expcor_mosaic_2/bk.reg")) :
        ...
        newfile = open("expcor_mosaic_2/bk.reg", "r")
        newfile.readline()
        f6 = newfile.read(6)
        if (f6 != "circle") :
    ...
    if (not os.path.isfile("expcor_mosaic_2/sources_mod.reg")) :
        ...
        newfile = open("expcor_mosaic_2/sources_mod.reg", "r")
        newfile.readline()
        f6 = newfile.read(6)
        if (f6 != "circle") :
    ...
    if (not os.path.isfile("expcor_mosaic/sources_mod.reg")) :
        ...
        newfile = open("expcor_mosaic/sources_mod.reg", "r")
        newfile.readline()
        f6 = newfile.read(6)
        if (f6 != "circle") :
