...
[accept_coordinates.txt](accept_coordinates.txt) is adapted the same as [accept_main.txt](accept_main.txt) above. This file is separate from the [accept_main.txt](accept_main.txt) file in order to easily compute column densities, seen in [accept_column_densities.txt](accept_column_densities.txt). This file is space delimited.
